...
export class RetirosService {
  ...
  constructor(private configService: ConfigService) {
    // Cloudinary ya fue inicializado globalmente por CloudinaryProvider en app.module.ts (DRY).
  ...
  async requestWithdrawal(
    ...
    if (errCuenta || !cuenta || cuenta.estado !== 'aprobada') {
      throw new BadRequestException(
        'La cuenta de retiro no está aprobada o no existe.',
      );
    }
    ...
    const { data: transaccion, error } = await this.supabase
      .from('transaccion')
      .insert({
        usuario_id: userId,
        tipo: 'retiro',
  ...
  async getPendingWithdrawals(limit: number = 20, offset: number = 0) {
    const { data, count, error } = await this.supabase
      .from('transaccion')
      .select(
        '*, usuario:usuario_id (nombre, apellido1, correo), cuenta_retiro:cuenta_retiro_id (*)',
        { count: 'exact' },
      )
      .eq('tipo', 'retiro')
      .eq('estado', 'pendiente')
  ...
  async processWithdrawal(
    ...
    if (!file)
      throw new BadRequestException(
        'Debe proporcionar un comprobante (imagen)',
      );
  ...
  async rejectWithdrawal(id: number, adminId: number) {
    ...
    if (transaccion.estado !== 'pendiente')
      throw new BadRequestException(
        'Solo se pueden rechazar retiros pendientes',
      );
