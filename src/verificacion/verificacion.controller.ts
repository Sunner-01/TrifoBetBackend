import { Controller, Get, Post, UseGuards, UseInterceptors, UploadedFiles, BadRequestException, Request } from '@nestjs/common';
...
export class VerificacionController {
  ...
  async getEstado(@Request() req) {
    ...
    if (!userId) {
      throw new BadRequestException('No se pudo identificar el usuario desde el token');
    }
  ...
  async subirDocumentos(
    ...
    if (!files || !files.anverso || !files.anverso[0] || !files.reverso || !files.reverso[0]) {
      throw new BadRequestException('Debe proporcionar las imágenes del anverso y reverso del documento.');
    }
    ...
    if (!allowedMimeTypes.includes(anverso.mimetype) || !allowedMimeTypes.includes(reverso.mimetype)) {
