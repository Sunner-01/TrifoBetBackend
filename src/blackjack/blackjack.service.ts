...
export class BlackjackService {
  ...
  private dealCardToPlayer(userId: string, handIndex: number): void {
    ...
    state.playerScores[handIndex] = this.calculateScore(
  ...
  takeInsurance(userId: string, wantsInsurance: boolean): Partial<GameState> {
    ...
    if (wantsInsurance) {
      ...
      if (state.balance < cost)
        throw new Error('Fondos insuficientes para seguro');
  ...
  stand(userId: string): Partial<GameState> {
    const state = this.getState(userId);
    if (
      state.handStatus[state.activeHandIndex] === 'playing' ||
      state.handStatus[state.activeHandIndex] === 'blackjack'
    ) {
      state.handStatus[state.activeHandIndex] =
        state.handStatus[state.activeHandIndex] === 'blackjack'
          ? 'blackjack'
          : 'stood';
    }
  ...
  double(userId: string): Partial<GameState> {
    ...
    if (state.balance < bet)
      throw new Error('Fondos insuficientes para doblar');
  ...
  split(userId: string): Partial<GameState> {
    ...
    const bet = state.handBets[state.activeHandIndex];
    if (state.balance < bet)
      throw new Error('Fondos insuficientes para dividir');
    ...
    state.playerScores[state.activeHandIndex] = this.calculateScore(
  ...
  async resolveGame(userId: string) {
    ...
    await this.updateBalanceAndLog(
      userId,
      profit,
      state.currentBet,
      overallResult,
    ...
    state.currentBet = 0; // Resetear la apuesta para obligar a hacer addBet en la siguiente
